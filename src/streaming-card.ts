...
export class FeishuStreamingSession {
  ...
  /**
   * Improved update with "immediate trigger" when throttle window has passed.
   * Based on AstrBot PR #5777 event-driven approach, adapted for TypeScript.
   * When enough time has passed since last update, send immediately without
   * going through the queue serialization overhead.
   */
  async update(text: string): Promise<void> {
    ...
    if (elapsed >= this.updateThrottleMs) {
      // Throttle window passed — send immediately (fire-and-forget for backpressure)
      // Reference: AstrBot send_streaming() uses RTT natural backpressure
      this.pendingText = null;
      this.lastUpdateTime = now;

      // Fire the update without awaiting — RTT natural backpressure
      // Any new content arriving mid-flight will trigger another update
      const textToSend = mergedInput;
      this.state.currentText = textToSend;
      this.updateCardContent(textToSend, (e) => this.log?.(`Update failed: ${String(e)}`));
      return;
    }

    // Within throttle window — buffer pending text, will be sent when window passes
    this.pendingText = mergedInput;

    // Enqueue a delayed send that fires after throttle window
    const delay = this.updateThrottleMs - elapsed;
    this.queue = this.queue.then(async () => {
      await new Promise((resolve) => setTimeout(resolve, delay));
      if (!this.state || this.closed) {
        return;
      }
      // Re-check: if more recent content arrived, use that instead
      const pending = this.pendingText ?? this.state.currentText;
      this.pendingText = null;
      if (pending === this.state.currentText) {
        return;
      }
      this.lastUpdateTime = Date.now();
      this.state.currentText = pending;
      await this.updateCardContent(pending, (e) => this.log?.(`Update failed: ${String(e)}`));
    });
  }
